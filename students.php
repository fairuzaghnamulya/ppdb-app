...
function getAllStudents($conn) {
    $stmt = $conn->prepare("SELECT u.id, u.username, cs.nama, cs.tanggal_lahir, cs.jenis_kelamin, cs.alamat, cs.no_telepon, u.created_at FROM users u JOIN calon_siswa cs ON u.id = cs.user_id WHERE u.role = 'siswa' ORDER BY u.created_at DESC");
    $stmt->execute();
...
<head>
    ...
    <title>Daftar Siswa - PPDB Online 2025</title>
    ...
    <div class="container mx-auto px-6 py-8 flex flex-col md:flex-row gap-8">
        ...
        <aside class="w-full md:w-64 flex-shrink-0">
            <div class="bg-white rounded-xl shadow-md p-6 sticky top-24">
                ...
                <nav class="space-y-2">
                    ...
                    <a href="students.php" class="flex items-center space-x-3 bg-sky-50 text-sky-600 font-medium px-4 py-3 rounded-lg">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <main class="flex-1">
            <div class="mb-8">
                ...
                <p class="text-gray-600 mt-2">Kelola data calon siswa yang telah mendaftar</p>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-md p-6">
                                ...
                                <?php foreach ($students as $student): ?>
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm font-medium text-gray-900"><?php echo htmlspecialchars($student['nama']); ?></div>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            <?php echo date('d F Y', strtotime($student['tanggal_lahir'])); ?>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            <?php echo htmlspecialchars($student['no_telepon']); ?>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                ...
                <?php else: ?>
                    <div class="text-center py-8">
                        ...
                        <p class="mt-4 text-gray-600">Belum ada siswa yang mendaftar</p>
                    </div>
                <?php endif; ?>
            </div>
        </main>
