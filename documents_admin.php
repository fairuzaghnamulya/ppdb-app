...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['document_id'])) {
    ...
    if ($status == 'approve' || $status == 'reject') {
        ...
        $stmt = $conn->prepare("UPDATE dokumen_siswa SET status_verifikasi = ? WHERE id = ?");
        $stmt->bind_param("si", $verification_status, $document_id);
...
function getAllDocuments($conn) {
    $stmt = $conn->prepare("SELECT ds.*, cs.nama FROM dokumen_siswa ds JOIN calon_siswa cs ON ds.calon_siswa_id = cs.id ORDER BY ds.id DESC");
    $stmt->execute();
...
<head>
    ...
    <title>Verifikasi Dokumen - PPDB Online 2025</title>
    ...
    <div class="container mx-auto px-6 py-8 flex flex-col md:flex-row gap-8">
        ...
        <aside class="w-full md:w-64 flex-shrink-0">
            <div class="bg-white rounded-xl shadow-md p-6 sticky top-24">
                ...
                <nav class="space-y-2">
                    ...
                    <a href="documents_admin.php" class="flex items-center space-x-3 bg-sky-50 text-sky-600 font-medium px-4 py-3 rounded-lg">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <main class="flex-1">
            <div class="mb-8">
                ...
                <p class="text-gray-600 mt-2">Kelola dan verifikasi dokumen yang diunggah oleh calon siswa</p>
            </div>
                                ...
                                <?php foreach ($documents as $document): ?>
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            <?php echo htmlspecialchars($document['nama']); ?>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <a href="<?php echo htmlspecialchars($document['file_path']); ?>" target="_blank" class="text-sky-600 hover:text-sky-900 mr-3">
                                                Lihat
                                            </a>
                                            
                                            <?php if ($document['status_verifikasi'] == 'pending'): ?>
                                                <form action="documents_admin.php" method="POST" class="inline">
